...
#define HEIGHT_FOG_DENCITY 0.002 //General height fog dencity multiplyer. [0 0.001 0.002 0.005 0.01 0.02 0.05 0.1 0.3 0.5 0.7 1.0]
...
#define H_FOG_MAX_DENCITY 5.0 //Max fog dencity. [0.01 0.02 0.05 0.1 0.2 0.5 0.7 0.9 1.0 2.0 5.0 10.0 20.0 50.0 100.0]
...
void main() {
    ...
    float max_d = H_FOG_MAX_DENCITY / ((HEIGHT_FOG_DENCITY + rainStrength * HEIGHT_FOG_RAIN_MULTIPLIER) / HEIGHT_FOG_DENCITY);
    float zero_height = log(max_d) / HEIGHT_FOG_DISTORTION + HEIGHT_FOG_SEA_LEVEL;
    //float zero_height = 0;
    ...
    float dencity = mean_exp_dencity * length(world_space) * (max_h - exp_bottom) / (max_h - min_h) + max_d * length(world_space) * (exp_bottom - min_h) / (max_h - min_h);
    //float dencity = 0.0;
    /* DRAWBUFFERS:8 */
